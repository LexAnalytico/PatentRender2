...
export async function POST(req: Request) {
  ...
  // Build a concise HTML email that highlights the requested fields: service category, payment cost, date, and status
  const paymentStatus = (paymentRow && (paymentRow.payment_status ?? paymentRow.status)) ?? 'N/A';
  ...
  const html = `
    ...
    <ul>
      ...
      <li><strong>Payment ID:</strong> ${razorpay_payment_id || payment_id || 'N/A'}</li>
    </ul>

    <p>If you have any questions, reply to this email and our team will assist you further.</p>
